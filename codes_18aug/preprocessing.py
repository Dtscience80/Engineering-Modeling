    ...
    if file_pro is not None:
        if checkbox_heatmap:
            ...
            if opsi_feature == "Semua Feature":
                ...
                if st.button("SHOW HEATMAP"):
                    ...
                    heatmap = sns.heatmap(
                        corr_matrix, annot=True, cmap="coolwarm", ax=ax
                    )
                ...
                nfeatures = st.number_input(
                    "Masukkan jumlah feature yang akan divisualisasikan dengan heatmap ",
                    0,
                )
        ...
        if checkbox_fs:
            ...
            feature_selection_results = pd.DataFrame(
                {"Feature": feature_names[fet_ind], "Importance": fet_imp}
            )

            st.write("Hasil feature selection : ")
            st.write(feature_selection_results)

            show_Nfeature = st.number_input(
                "Masukkan jumlah feature teratas yang ingin didapatkan : ", 0
            )
            ...
            opsi_save = ["Ya", "Tidak"]
            save_dataset = st.selectbox(
                "Apakah feature hasil seleksi akan disimpan ke dalam dataset sebelumnya?",
                opsi_save,
            )
        ...
        if checkbox_normalisasi:
            ...
            if norm_opsi == "Feature pilihan":
                nfeatures = st.number_input(
                    "Masukkan banyak feature pada dataset yang akan dilakukan normalisasi : ",
                    0,
                )
                # List untuk menyimpan fitur
                features_normalized = []
